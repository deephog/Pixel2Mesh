...
class VGG16P2M(nn.Module):
    ...
    def __init__(self, n_classes_input=1, pretrained=False):
        ...
        feat_base = 16
        self.conv0_1 = nn.Conv2d(n_classes_input, feat_base, 3, stride=1, padding=1)
        self.conv0_2 = nn.Conv2d(feat_base, feat_base, 3, stride=1, padding=1)

        # 448 - 224 - 112 - 56 - 28 - 14
        # 256 - 128 - 64 - 32 - 16 - 8
        self.conv1_1 = nn.Conv2d(feat_base, feat_base * 2, 3, stride=2, padding=1)  # 224 -> 112
        self.conv1_2 = nn.Conv2d(feat_base*2, feat_base*2, 3, stride=1, padding=1)
        self.conv1_3 = nn.Conv2d(feat_base*2, feat_base*2, 3, stride=1, padding=1)

        self.conv2_1 = nn.Conv2d(feat_base*2, feat_base*4, 3, stride=2, padding=1)  # 112 -> 56
        self.conv2_2 = nn.Conv2d(feat_base*4, feat_base*4, 3, stride=1, padding=1)
        self.conv2_3 = nn.Conv2d(feat_base*4, feat_base*4, 3, stride=1, padding=1)

        self.conv3_1 = nn.Conv2d(feat_base*4, feat_base*8, 3, stride=2, padding=1)  # 56 -> 28
        self.conv3_2 = nn.Conv2d(feat_base*8, feat_base*8, 3, stride=1, padding=1)
        self.conv3_3 = nn.Conv2d(feat_base*8, feat_base*8, 3, stride=1, padding=1)

        self.conv4_1 = nn.Conv2d(feat_base*8, feat_base*16, 5, stride=2, padding=2)  # 28 -> 14
        self.conv4_2 = nn.Conv2d(feat_base*16, feat_base*16, 3, stride=1, padding=1)
        self.conv4_3 = nn.Conv2d(feat_base*16, feat_base*16, 3, stride=1, padding=1)

        self.conv5_1 = nn.Conv2d(feat_base*16, feat_base*32, 5, stride=2, padding=2)  # 14 -> 7
        self.conv5_2 = nn.Conv2d(feat_base*32, feat_base*32, 3, stride=1, padding=1)
        self.conv5_3 = nn.Conv2d(feat_base*32, feat_base*32, 3, stride=1, padding=1)
        self.conv5_4 = nn.Conv2d(feat_base*32, feat_base*32, 3, stride=1, padding=1)
    ...
    def forward(self, img):
        ...
        return [torch.cat([img_orig, img0], dim=1), img1, img2, img3, img4, img5]




class VGG16Mesh(nn.Module):

    def __init__(self, n_classes_input=1, pretrained=False):
        super(VGG16Mesh, self).__init__()
    ...
    def forward(self, img):
        ...
        return [img_orig, img0, img1, img2]#, img5]


class VGG16Recons(nn.Module):

    def __init__(self, input_dim=128, image_channel=3):
        super(VGG16Recons, self).__init__()

        self.conv_1 = nn.ConvTranspose2d(512, 256, kernel_size=2, stride=2, padding=0)  # 14 -> 28
        self.conv_2 = nn.ConvTranspose2d(512, 128, kernel_size=4, stride=2, padding=1)  # 28 -> 56
        self.conv_3 = nn.ConvTranspose2d(256, 64, kernel_size=4, stride=2, padding=1)  # 56 -> 112
        self.conv_4 = nn.ConvTranspose2d(128, 32, kernel_size=6, stride=2, padding=2)  # 112 -> 224
        self.conv_5 = nn.ConvTranspose2d(64, 16, kernel_size=6, stride=2, padding=2)  # 224 -> 448
        self.conv_6 = nn.Conv2d(32, image_channel, kernel_size=1, stride=1)
        # self.conv_6 = nn.ConvTranspose2d(32, 16, kernel_size=6, stride=2, padding=2)  # 448 -> 896
        # self.conv_7 = nn.ConvTranspose2d(16, image_channel, kernel_size=6, stride=2, padding=2)  # 896 -> 1792
    ...
    def forward(self, img_feats):
        ...
        x = torch.cat((x, img_feats[-5]), dim=1)
        x = F.relu(self.conv_5(x))
        x = torch.cat((x, img_feats[-6]), dim=1)
        x = self.conv_6(x)
        # x = self.conv_7(x)
